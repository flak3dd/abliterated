...
| **Abliterated IDE** | **53.0** | Local bridge workbench; strong apply protocol + loop UX; missing MCP/cloud/Tab/browser |
...
| 11 | Real background job runners (not UI mock) | N | Y | Y | Y | Y | P | P | Y |
...
| 25 | MCP client (extensible tools) | N | Y | Y | Y | Y | Y | Y | Y |
...
| 40 | `git_diff` / structured diff tool | N | Y | Y | Y | Y | Y | Y | Y |
| 41 | Create / open pull request | N | Y | Y | Y | Y | P | P | N |
| 42 | Non-git checkpoints / restore | N | Y | P | P | P | N | P | N |
...
| # | Capability | Abl | Cur | CC | Cop | Cdx | Gem | Win | Grk |
| ---: | --- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| 46 | MCP servers | N | Y | Y | Y | Y | Y | Y | Y |
...
| 57 | Headless / CI scriptable agent | N | P | Y | P | Y | Y | N | Y |
...
| 67 | Jobs UI backed by real workers | N | Y | Y | Y | Y | P | P | Y |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| **Score /75** | **53.0** | **66.5** | **60.5** | **58.5** | **62.0** | **50.0** | **49.5** | **53.5** |
...
`read_file`, `grep`, `glob`, `list_dir`, `file_outline`, `semantic_search` (lexical+path ranking — **not** embeddings), `git_status`, `git_commit`, `shell`, `web_fetch`, `generate_image` (only if Images enabled).
...
| --- | --- |
| MCP client | **Missing** |
| Cloud / background agents | **Missing** (Jobs screen is **dummy** localStorage only) |
| Checkpoints / create_PR / git_diff tool | **Missing** |
