/**
 * Client-Side Single File & ZIP Archive Downloader
...
export function detectFilenameAndContent(
  ...
  // Patterns for file path annotations:
  // e.g. "// ai-pentest/requirements.txt", "# filepath: src/app.py", "/* styles.css */", "### config.yaml"
  const patterns = [
  ...
  for (const pattern of patterns) {
    ...
    if (match1 && match1[1]) {
      ...
      // Remove annotation line from code
...
/**
 * Extracts all downloadable code blocks and files from markdown content
 */
export function extractFilesFromMarkdown(markdown: string): DownloadableFile[] {
  ...
  while ((match = regex.exec(markdown)) !== null) {
    ...
    // Check if header itself contains a filepath, e.g. ```python:src/engine.py or ```ts src/main.ts
    let headerFile = '';
    let lang = rawHeader;
