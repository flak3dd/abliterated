...
Abliterated’s image stack maps to one Custom app for images (optional second for Qwen):
...
`port-7860.bash` starts the Diffusers OpenAI bridge only (`krea2-raw-fp8`). ComfyUI / `:8188` is removed.
...
4. Click the app on the device window. Abliterated can use `http://127.0.0.1:7860/v1` with Via proxy on, or the Spark LAN IP with proxy off.
...
That writes `~/.config/NVIDIA/Sync/bin/scripts/port-{7860,8000}.bash` (the path Sync’s CLI also looks for).
